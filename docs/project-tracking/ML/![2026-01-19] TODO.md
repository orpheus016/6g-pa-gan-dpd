...
- [ ] FIX jupyter notebook to match train.py and ARCHITECTURE.md (memory depth M=3, phase-normalized features, no Tanh at output)
    ...
    - [ ] Compare spectral loss computation with metrics computation opendpd and also verify if the implementation is correct since the dataset is 5 channel while on previous notebook only one channel exists
